...
void page_104(Player &player) {
  std::cout << "\n\nThe BANDIT woman steps forward with her sword raised, "
               "shouting 'Death to the intruder!' She is their leader and you "
               "must fight her.\n";
  std::cout << "\n\nBANDIT LEADER --- SKILL 8 --- STAMINA 6\n";
  Enemy bandit_leader("BANDIT LEADER", 8, 6);
  bool result = battle(player, bandit_leader, -1);
  if (result) {
    std::cout
        << "\n\nIf you manage to defeat her, you must now fight the other four "
           "Bandits in pairs. Both members of a pair will have a separate "
           "attack on you in each Attack Round, but you must choose which of "
           "the two you will fight. Attack your chosen  Bandit as in a normal "
           "battle. Against the other you will throw for your Attack Strength "
           "in the normal way, but you will not wound it if your Attack "
           "STrength is greater; you must count this as though you have just "
           "fended off his blow. Of course if his Attack Strength is greater, "
           "he will have wounded you in the normal way.\n";

    std::cout << "\n\nBANDIT A --- SKILL 7 --- STAMINA 6\n";
    std::cout << "\n\nBANDIT B --- SKILL 6 --- STAMINA 4\n";
    Enemy bandit1("BANDIT A", 7, 6);
    Enemy bandit2("BANDIT B", 6, 4);
    battle(player, bandit1, -1);
    battle(player, bandit2, -1);

    std::cout << "\n\nBANDIT C --- SKILL 7 --- STAMINA 5\n";
    std::cout << "\n\nBANDIT D --- SKILL 5 --- STAMINA 4\n";
    Enemy bandit3("BANDIT C", 7, 5);
    Enemy bandit4("BANDIT D", 5, 6);
    battle(player, bandit3, -1);
    battle(player, bandit4, -1);
    page_311(player);
  } else {
    std::cout << "\n\nYou have perished against the bandits. Your adventure "
                 "ends here.\n";
  }
...
void page_126(Player &player) {
  ...
  for (int i = 0; i <= player.equipment.size(); i++) {
    if (player.equipment[i].getName() == "Nose Filters") {
      std::cout << "\n1. Use the Nose Filters?\n2. Choose not to use them.\n";
      int pick = choice();
      switch (pick) {
      case 1:
        player.equipment.erase(player.equipment.begin() + i - 1);
...
void page_141(Player &player) {
  ...
  do {
    std::cout
        << "\n\nYou step around the bulbous mass of the Giant Spider and "
           "settle down again to a nervous sleep. You awake eraly in the "
           "morning and set off north along the path. The ground is quite "
           "steep now as the path wends its way up into the hills. By the time "
           "you reach the top the sun is quite hot. All around in the distance "
           "you see the tight green circle of Darkwood Forest. Mist still "
           "hangs in the tall grasses behind you but ahead you see a valley "
           "floor bathed in sunlight. All is  quiet. As you start down the far "
           "side of the hill, you see a junction in the path. You may:\n\n1. "
           "Continue north down the hill.\n2. Go east along the new path.\n3. "
           "View INVENTORY\n";
...
void page_163(Player &player) {
  std::cout
      << "\n\nAlthough it is a bright sunny day a tiny grey cloud appears in "
         "the sky. It is very low and appears to be moving towards you. As it "
         "approaches you see that it is moving very quickly. Finally it hovers "
         "above you some five metres off the ground. Suddenly a bolt of "
         "lighting shoots out from the cloud, hitting you on the "
         "shoulder.\n\nLose 3 Stamina points.\n\n ";

  player.removeStamina(3);

  std::cout
      << "If you are still alive you "
         "see the cloud shoot off to the west at high speed. You pull yourself "
         "together and set off north again.\n";
  next();
  page_375(player);
}
void page_164(Player &player) {
...
void page_195(Player &player) {
  ...
  do {
    std::cout
        << "\n\nYou reach the top of the vine and scramble on to a wooden "
           "platform. A sheet made from leaves and ferns covers the entrance "
           "to a small covered living area. As you approach, the sheet is "
           "trhwon back, and from behind it steps a large and hairy ape-like "
           "creature wearing only and animal-hide loin cloth. He is holding a "
           "large bone in his right hand and grunts at you. He is an APE MAN. "
           "You may:\n1. Draw your sword and attack him.\n2. Jump off the "
           "platform to the ground five metres below.\n3. "
           "View INVENTORY\n";
...
void page_196(Player &player) {
  ...
  do {
    std::cout
        << "\n\nYou step into the dark cavern with your sword drawn. The air "
           "is cold and moist. You hear a loud snoring and as your eyes adjust "
           "to the darkness you see the large bulk of a CAVE TROLL asleep in a "
           "large stone chair. His skin is brown and wrinkled, and he is "
           "wearing animal hides. A wooden club lies across his lap and a "
           "large leather bag hangs on the back of the stone chair. Will "
           "you:\n1. Creep up and take the leather bag while the Cave Troll "
           "sleeps?\n2. Take out the Net of Entanglement (if you have it) from "
           "your backpack?\n3. Walk back to the junction and head north?\n4. "
           "View INVENTORY\n";
    ...
    case 2:
      ...
      break; // Exit the loop and function after making a choice
    case 3: {
      std::cout << "\n\nDo you possess an Net of Entanglement?\n";
      for (int i = 0; i <= player.equipment.size(); i++) {
        if (player.equipment[i].getName() == "Net of Entanglement") {
          std::cout
              << "\n1. Net of Entanglement?\n2. Creep up and take the leather "
                 "bag while the Cave Troll "
                 "sleeps?\n3. Walk back to the junction and head north?\n";
          int pick = choice();
          switch (pick) {
          case 1:
            player.equipment.erase(player.equipment.begin() + i - 1);
            std::cout << "\n\nNet of Entanglement Consumed.";
            page_39(player);
            break;
          case 2:
      ...
      std::cout << "\n\nYou do not possess the Net of Entanglement\n";
      next();
...
void page_197(Player &player) {
  std::cout << "\n\nDo you possess an Potion of Anti-Poison?\n";
  for (int i = 0; i <= player.equipment.size(); i++) {
    if (player.equipment[i].getName() == "Potion of Anti-Poison") {
      std::cout
          << "\n1. Use Potion of Anti-Poison?\n2. Disregard the potion.\n";
      int pick = choice();
      switch (pick) {
      case 1:
        player.equipment.erase(player.equipment.begin() + i - 1);
  ...
  std::cout << "\n\nYou do not possess the Potion of Anti-Poison\n";
  next();
}
void page_198(Player &player) {
  int pick = 0; // Declare pick outside the loop to persist between iterations
  do {
    std::cout
        << "\n\nThe ground is quite steep as the path wends its way into the "
           "hills. By the time you reach the top the sun is quite hot. All "
           "around in the distance you see the dark green circle of Darkwood "
           "Forest. Mist still hangs in the tall grasses behind you but ahead "
           "you see a valley floor bathed in sunlight. All is quiet. As you "
           "start down the far side of the hill you see a junction in the "
           "path.\n1. You may either continue north down the hill.\n2. Or head "
           "east along the new branch.\n3. "
           "View INVENTORY\n";
...
void page_199(Player &player) {
  int pick = 0; // Declare pick outside the loop to persist between iterations
  do {
    std::cout << "\n\nTo the left of the path you notice a large mud pool "
                 "bubbling loudly. Steam rises from the tick bubbles breaking "
                 "the surface of the mud. You may:\n1. Throw a Gold Piece in "
                 "the mud and make a wish.\n2. Rub some hot mud on your "
                 "wounds.\n3. Continue north along the path\n4. "
                 "View INVENTORY\n";
...
void page_200(Player &player) {
  int pick = 0; // Declare pick outside the loop to persist between iterations
  do {
    std::cout << "\n\nYou take the small silver key from out of your backpack "
                 "and place it into the keyhole. It fits perfectly and you "
                 "give it a turn. The locks clicks and the stone door swings "
                 "open. Stone s tairs lead down from the door into gloomy "
                 "depths. You cannot see a thing  dwond the stairs.\n1. If you "
                 "wish to decend the stairs.\n2. If you wish to leave the "
                 "building, return to the path and head north.\n3. "
                 "View INVENTORY\n";
...
void page_201(Player &player) {
  int pick = 0; // Declare pick outside the loop to persist between iterations
  do {
    std::cout << "\n\nIf you have not done so already\n1. You may try to open "
                 "the wooden chest.\n2. Alternatively you may leave the alcove "
                 "to climb further up the steps.\n3. "
                 "View INVENTORY\n";
...
void page_202(Player &player) {
  std::cout << "\n\nThe ears of the Catwoman are pierced by two gold studs. "
               "They are worth 5 Gold Pieces each and you put them into your "
               "backpack. You set off east along the twisting path.\n";
...
void page_203(Player &player) {
  std::cout << "\n\nAs you start to speak the Goblin looks up and smiles. Then "
               "he starts to metamorphose before your eyes. He becomes taller "
               "and turns green. A large spiny tail extends from his back, his "
               "arms thicken and his hands grow sharp claws. His face distorts "
               "and becomes reptilian with red eyes, a wide mouth and dozens "
               "of razer-sharp teeth. He is not a Goblin but a SHAPE CHANGER, "
               "and you must fight him.\n";
  std::cout << "\n\n SHAPE CHANGER --- SKILL 10 --- STAMINA 10\n";
  Enemy Gremlin1("SHAPE CHANGER", 10, 10);
  battle(player, Gremlin1, -1);
  next();
  page_373(player);
}
void page_204(Player &player) {
  ...
  std::cout
      << "\n\nThe path leads through the field to a stone bridge over a "
         "clear stream. Beyond the bridge are the small cottages and wooden "
         "huts of a village. A sign on the bridge reads 'Stonebridge'. You "
         "cross the bridge and see two old dwarfs with long white beards "
         "standing by a cottage looking at you. Do you have the hammer head "
         "and handle with the letter G inscribed in them?\n";
  for (int i = 0; i <= player.equipment.size(); i++) {
    if (player.equipment[i].getName() ==
        "BRONZE HAMMER HEAD with letter G inscribed") {
      if (player.equipment[i].getName() ==
          "BRONZE HAMMER HANDLE with letter G inscribed") {
        next();
        page_400(player);
      }
    }
...
void page_209(Player &player) {
  ...
  do {
    std::cout << "\n\nAs you enter the hut the big man smiles. He looks "
                 "pleased to see you and starts to speark in a deep voice. "
                 "'Welcome stranger. My name is Quin and I earn my living by "
                 "my arms. Would you care for a little wager perhaps at arm "
                 "wrestling?'\n1. If you wish to accept the challenge.\n2. If "
                 "you wish to decline his offer, you politely refuse and "
                 "return to the junction in the path.\n3. View INVENTORY";
...
void page_210(Player &player) {
  int pick = 0; // Declare pick outside the loop to persist between iterations
  do {
    std::cout << "\n\nThe men curse and jump up and down in anger at having "
                 "missed you. They start to argue and push each other around. "
                 "They seem to forget all about you.\n1. If you want to attack "
                 "the  WILD HILL MEN.\n2. If you want to walk past them along "
                 "the path while they are still arguing.\n3. View INVENTORY";
...
void page_211(Player &player) {
  std::cout << "\n\nYou take the small bottle labelled Potion of Anti-Poison "
               "from your backpack and gulp down the contents. The pain in "
               "your stomach gradually fades away and you relax. There does "
               "not appear to be much sense in staying here any longer and you "
               "walk over to the steps on the far wall.\n";
  next();
  page_293(player);
}
void page_212(Player &player) {}
void page_213(Player &player) {
  std::cout
      << "\n\nThe path twists and turns between the trees and bushes, and then "
         "comes to another junction. You realize that the way south leads back "
         "to the valley and so you decide to ignore it and head north.\n";
  next();
  page_305(player);
}
void page_214(Player &player) {
  std::cout << "\n\nYou reach into your backpack and uncork the small bottle "
               "containing the healing potion. The pain in your leg is "
               "agonizing but rapidly dies down as the potion takes effect. "
               "Soon your leg is fully mended and you are able to stand on "
               "your feet. Your eyes are now accustomed to the dark and you "
               "see that the tunnel is only one metre high and you must crawl "
               "along on your hands and knees to explore it.\n";
  next();
  page_69(player);
}
void page_215(Player &player) {
  int pick = 0; // Declare pick outside the loop to persist between iterations
  do {
    std::cout << "\n\nInside the barrel you find a heavy iron shield.\n1. If "
                 "you wish to take it with you.\n2. If you do not wish to "
                 "touch it.\n3. View INVENTORY";
...
void page_216(Player &player) {
  std::cout << "\n\nThe path leads along a ridge of the hill and ends at "
               "another junction. You see that the way south leads back to the "
               "river, so you decide to head north again.\n";
  next();
  page_278(player);
...
void page_217(Player &player) {
  ...
  do {
    std::cout
        << "\n\nYou walk around the lifelss bulk of the worm to examine the "
           "contents of its lair. There are several skeletons, perhaps those "
           "belonging to other unfortunate adventurers. By the side of one of "
           "them you find a leather backpack. Inside the backpack you find 4 "
           "Gold Pieces and a small, corked bottle containing a colorless "
           "liquid.\n1. If you wish to drink the contents of the bottle.\n2. "
           "If you would rather leave the dark cavern and scramble back up to "
           "the path again taking just the gold with you.\n3. View INVENTORY";
    player.addGold(4);
    pick = choice();
...
void page_219(Player &player) {
  std::cout
      << "\n\nYou notice that the small dart that protrudes from the Bear's "
         "chest is made of silver. Its value is 5 Gold Pieces and you may put "
         "it in your backpack if you wish. Add 1 LUCK point for your "
         "discovery. You now return to the path and head north\n";
  player.addLuck(1);
  player.addGold(5);
  next();
  page_300(player);
...
void page_224(Player &player) {
  std::cout
      << "\n\nOne  of the Death Hawks has a silver band around one of its "
         "legs. It has an inscription on it which reads 'Death awaits you'. "
         "You decide to leave the silver band and head quickly west.\n";
  next();
  page_332(player);
}
void page_225(Player &player) {
  ...
  do {
    std::cout
        << "\n\nYou curse and pull yourself out of the water on to the "
           "entrance of the tunnel. You are  suprised to see that it is lit up "
           "by torches at rgular intervals along its legnth. \n1. If you wish "
           "to crawl along the tunnel.\n2. If you wish to climb back up the "
           "ladder and return to the path.\n3. View INVENTORY";
...
void page_228(Player &player) {
  std::cout
      << "\n\nThe pit is circular, with smooth sides, and you are weak from "
         "your fall. You reach into your backpack and pull out the brown "
         "leather boots. They are very light on your feet. You crouch down and "
         "in one mighty leap you are out of the pit. You dust yourself off and "
         "continue your walk north down the gorge.\n";
  next();
  page_255(player);
}
void page_229(Player &player) {}
void page_230(Player &player) {
  ...
  do {
    std::cout
        << "\n\nSlowly you peer into the cave and see the huge shape of an "
           "OGRE walking slowly over to a wicker cage with a bowl of water in "
           "his great hand. He is dressed in animal furs and carries a stone "
           "club in his belt. There appears to be a small creature jumping "
           "around inside the cage. You may:\n1. Pick up a rock and throw it "
           "at the Ogre.\n2. Rush in and attack the Ogre with your sword.\n3. "
           "Leave the cave and continue up the path.\n4. View INVENTORY";
...
void page_231(Player &player) {
  std::cout
      << "\n\nThe path twists and turns and then makes a sudden sharp turn to "
         "the west. Following the new course you are aware of squaking in the "
         "trees all around you. You hear the flapping of wings and look up to "
         "see three large birds swooping down on you. THeir beaks and talons "
         "look razor-sharp. You only have a second in which to draw your "
         "sword to defend yourself against the DEATH HAWKS.\n";
  std::cout << "\n\nFirst DEATH HAWK --- SKILL 4 --- STAMINA 4\n";
  Enemy deathhawk1("First DEATH HAWK", 4, 4);
  bool result1 = battle(player, deathhawk1, 1);
  if (result1) {
    std::cout << "\n\nSecond DEATH HAWK --- SKILL 4 --- STAMINA 3\n";
    Enemy deathhawk2("Second DEATH HAWK", 4, 3);
    bool result2 = battle(player, deathhawk2, 1);
    if (result2) {
      std::cout << "\n\nThird DEATH HAWK --- SKILL 5 --- STAMINA 4\n";
      Enemy deathhawk3("Third DEATH HAWK", 5, 4);
      bool result3 = battle(player, deathhawk3, 1);
...
void page_232(Player &player) {
  std::cout
      << "\n\nYou bend down over the lifelss body of the mad Goblin and "
         "examine the rod around its neck. The rod is made of ebony and there "
         "is a screw thread at one end. You are excited to see the letter G "
         "neatly inscribed at the other end of what must be the handle of the "
         "dwarfish war-hammer. You put your find in your backpack. Add 1 LUCK "
         "point.\n\nBRONZE HAMMER HANDLE "
         "with letter G inscribed added to your Inventory!\n1 LUCK point "
         "Added.\n";
  player.addLuck(1);
  player.equipment.push_back(
      {"BRONZE HAMMER HANDLE with letter G inscribed", 200});

  int pick = 0; // Declare pick outside the loop to persist between iterations
  do {
    std::cout << "\n\n1. If you wish to search through the contents of the "
                 "cave.\n2. If you want to leave the cave and continue "
                 "northwards.\n3. View INVENTORY";
...
void page_235(Player &player) {
  std::cout << "\n\nYou reach quickly into your backpack and pull out the "
               "small corked bottle containing the potion. You gulp down the "
               "liquid. Calm spreads through your body despite the chaos all "
               "around you. Suddenly the hut collapses and crashes to the "
               "ground. You decide it is time to get out of this place and run "
               "back to the path to head north.\n\nAdd 1 LUCK point\n";
  next();
  page_149(player);
}
void page_236(Player &player) {
  ...
  do {
    std::cout
        << "\n\nThe path ends outside a stone cottage with a thatched roof. A "
           "plaque above the wodden door reads 'Arragon the Arch-Mage'.\n1. If "
           "you wish to enter the cottage.\n2. If you would rather return to "
           "the junction in the path and head north.\n3. View INVENTORY";
...
void page_241(Player &player) {
  ...
  std::cout
      << "\n\nAs you wend your way through the trees in the direction of the "
         "growling, you  suddenly come face to face witha huge, brown BEAR. A "
         "small dart protrudes from its chest and it looks mad with pain and "
         "rage. You draw your sword and prepare to fight.\n1.\n";

  std::cout << "\n\nBEAR --- SKILL 7 --- STAMINA 8\n";
  Enemy bear("BEAR", 7, 8);
  battle(player, bear, -1);
  next();
  page_219(player);
  return;
}
void page_242(Player &player) {
  ...
  do {
    std::cout
        << "\n\nRummaging through some bedding in a chest of drawers you find "
           "an "
           "ingot of gold. It is worth 28 Gold Pieces. However it is very "
           "heavy "
           "and if you wish to take it with you, some other object from your "
           "backpack will have to be discarded.\n1. Discard an object from "
           "your backpack\n2. Leave the Ingot and continue onwards.\n";

    pick = choice();

    switch (pick) {
    case 1: {
      // list items in backpack and let user choose which to remove
      std::cout << "\t     ITEM\t\t\t\t\t     COST" << std::endl;
      for (int i = 0; i < player.equipment.size(); i++) {
        std::cout << i + 1 << ". " << player.equipment[i];
      }
      int pick2 = choice();
      ...
      for (int i = 0; i <= player.equipment.size(); i++) {
        if (pick2 == i) {
          std::cout << "\n\n"
                    ...
                    << " removed.\n\n Gold Ingot added.\n";
          player.equipment.erase(player.equipment.begin() + i - 1);
          player.equipment.push_back({"Gold Ingot", 28});
        }
      }
      break; // Exit the loop and function after making a choice
    }
  ...
  } while (pick != 1); // Loop until player picks WEST or EAST
  std::cout
      << "\n\nThere is no other exit from "
         "the "
         "cave and you have to crawl back down the tunnel to the junction.\n";
...
void page_243(Player &player) {
  int pick = 0; // Declare pick outside the loop to persist between iterations
  do {
    std::cout << "\n\nYou tell him that you are sorry, but you have not seen "
                 "his brass bell. The poor old friar frowns and then ask you "
                 "if you would like to give him 1 Gold Piece for a good "
                 "cause.\n1. If you do.\n2. If you do not wish to make the "
                 "donation.\n3. View INVENTORY";
...
void page_244(Player &player) {
  std::cout << "\n\nThe effects the fever finally wears off and you gratefully "
               "fall asleep again. In the morning you collect your belongings "
               "and head north along the path to the hills.\n";
  next();
  page_198(player);
}
void page_245(Player &player) {
  int pick = 0; // Declare pick outside the loop to persist between iterations
  do {
    std::cout << "\n\nArriving on the valley floor you find that the path "
                 "splits.\n1. To continue north\n2. To go west\n3. To go "
                 "east\n3. View INVENTORY";
...
void page_246(Player &player) {
  std::cout << "\n\nThe BANDIT woman takes the items from you and steps back "
               "to let you pass. You head north again and soon notice the "
               "trees beginning to thin out on either side of the path. "
               "Eventually the path leads out of the trees into a ploughed "
               "field. You are out of Darkwood Forest!\n";
  next();
  page_204(player);
}
void page_247(Player &player) {
  ...
  do {
    std::cout
        << "\n\nThe loathsome Pterodactyl lies in a crumpled heap where it "
           "crashed to the floor after the fatal blow from your sword. Walking "
           "over to it you notice a yellow arrow painted on the grass beside "
           "the path, pointing west.\n1. If you wish to follow the arrow.\n2. "
           "If you wish to keep going north along the path.\n3. View INVENTORY";
...
void page_248(Player &player) {
  ...
  do {
    std::cout
        << "\n\nYou now possess and emperor's shield made long ago by a master "
           "armourer. \n\nAdded 1 LUCK point.\n\nThe shield will give you "
           "greater defence in all future battles. SHould a creature woud you, "
           "throw one die. If you throw a 4, 5, or 6, its damage to you will "
           "be reduced by 1 point.\n1. If you have not done so already, you "
           "may now either try to open the wooden chest.\n2. Or leave the "
           "alcove to climb further up the steps.\n3. View INVENTORY";
...
void page_249(Player &player) {
  std::cout << "\n\nYou look back into the strange cavern and see the humanoid "
               "clone workers continuing their labours in the fungi field. You "
               "shake your head in disbelief and run up the last few steps to "
               "the hole in the cavern roof.\n";
  next();
  page_164(player);
}
void page_250(Player &player) {
  std::cout << "\n\nThe vase drops to the ground but does not break although "
               "cracks appear all over it. You begin to feel a vibration and "
               "then you notice cracks appearing all over the porch and the "
               "sides of the hut. The vibrations become stronger - your whole "
               "body starts to tremble and your head feels as if it is about "
               "to explode.\n\nLose 2 STAMINA point.\n";
  next();
  page_82(player);
}
void page_251(Player &player) {
  std::cout << "\n\nAs you draw your sword the Centaur reaches for his bow and "
               "arrows. Before you can reach him he lets fly an arrow straight "
               "at you.\n\nRoll two dice.\n";
  next();
  int playerRoll = (rand() % 6 + 1) + (rand() % 6 + 1);
  std::cout << "\n\nYou rolled a: " << playerRoll;
  if (playerRoll <= player.currSkill) {
    // LUCKY
    std::cout << "\n\nLucky! You manage to dodge the arrow.\n";
    player.removeLuck(1);
    next();
    page_63(player);
  } else {
    // UNLUCKY
    std::cout << "\n\nUnlucky. You are unable to dodge the arrow which lodges "
                 "itself into your shoulder. Lose 4 STAMINA points.\n";
    player.removeStamina(4);
  }
  next();
  std::cout << "\n\nIf you are still alive, you pull out the arrow, painfully, "
...
void page_252(Player &player) {
  ...
  do {
    std::cout
        << "\n\nYou arrive at a four-way junction in the path. South leads "
           "back to the forest so you discount that option.\n1. If you wish to "
           "head north.\n2. If you wish to continue east.\n3. View INVENTORY";
...
void page_255(Player &player) {
  ...
  do {
    std::cout
        << "\n\nContinuing down the gorge you see the handle of a sword "
           "sticking out form a large rock by the side of the path.\n1. If you "
           "wish to try to pull the sword free from the rock.\n2. If you wish "
           "to continue walking down the gorge.\n3. View INVENTORY";
...
void page_256(Player &player) {
  std::cout
      << "\n\nClimbing down the ladder you do not notice that a rung is "
         "missing. You slip and lose your footing.\n\nTest your Luck.\n\n";
  ...
  if (playerRoll <= player.currLuck) {
    // LUCKY
    std::cout
        << "\n\nLucky! You manage to hang on the ladder with your hands.\n";
    player.removeLuck(1);
    next();
    page_122(player);
  } else {
    // UNLUCKY
    std::cout << "\n\nUnlucky. You fall off the ladder and plunge to the water "
                 "below.\n";
  }
  player.removeLuck(1);
  next();
...
void page_257(Player &player) {
  std::cout << "\n\nInside the backpack you find some elven bread which will "
               "restore 4 STAMINA points if you eat it.\n\nAdd 1 LUCK point\n";
  next();
  page_31(player);
}
void page_258(Player &player) {
  std::cout << "\n\nYou reach into your backpack and pull out the tiny brass "
               "flute. You have a strange feeling that you must play it now, "
               "before the enraged Wyvern. As you do, a soft and gentle sound "
               "plays out from the flute and a quizzical look appears on the "
               "Wyvern's face. Its mouth closes and its eyelids start to  "
               "droop. You are playing a magical flute of Dragonsleep and the "
               "Wyvern is powerless to resist its soothing song. Slowly the "
               "Wyvern slumps to the ground and is soon fast asleep.\n";
  next();
  page_305(player);
}
void page_259(Player &player) {

  std::cout << "\n\nYou feel a burning sensation inside your body as you sink "
               "into a raging fever. You may be about to change into a "
               "Werewolf yourself!\n\nLose 3 STAMINA points\n";
  ...
  if (playerRoll <= player.currLuck) {
    // LUCKY
    std::cout << "\n\nLucky! Your fever dies down and you did not turn into a "
                 "Werewolf.\n";
    player.removeLuck(1);
    next();
    page_244(player);
    ...
    // UNLUCKY
    std::cout
        << "\n\nUnlucky. The fever continues to group your body and you are "
           "horrified ot see thick brown hair appearing on the back of your "
           "hands. Shock causes you to lose 2 additional STAMINA points.\n";
    player.removeStamina(2);
  }
  player.removeLuck(1);
  next();
...
void page_272(Player &player) {
  std::cout << "\n\nThat is the last time you are going to do anybody a favour "
               "for some time! Returning to the path you head north again.\n";
  next();
  page_394(player);
}
void page_273(Player &player) {
  std::cout
      << "\n\nYou take the Medallion from the neck of the fallen Gremlin. It "
         "is made of gold and is worth 9 Gold Pieces. There is nothingelse of "
         "use or value in the cave. There is not other exit from the cave and "
         "you must crawl back down the tunnel to the junction.\n";
  player.equipment.push_back({"Medallion", 9});
  next();
  page_296(player);
}
void page_274(Player &player) {
  int pick = 0; // Declare pick outside the loop to persist between iterations
  do {
    std::cout << "\n\nYou notice a knotted vine hanging down to the ground "
                 "from a tree on your left. You look up and see a roughly made "
                 "tree house amid the breanches.\n1. If you want to climb up "
                 "the vine to the tree house.\n2. If you wish to continue "
                 "walking north.\n3. View INVENTORY";
...
void page_276(Player &player) {
  std::cout
      << "\n\nAs you step back to attack the masked rider, the largest of the "
         "four dogs leaps at you and you are forced to fight it fight.\n";
  std::cout << "\n\nHUNTING DOG --- SKILL 7 --- STAMINA 6\n";
  Enemy hunting_dog("HUNTING DOG", 7, 6);
  bool result = battle(player, hunting_dog, -1);
  if (result) {
    std::cout
        << "\n\nIf you defeat the Hunting Dog, you must not fight the other "
           "three dogs and their master in pairs. Both members of a pair will "
           "have a separate attack no you in each Attack Round, but you must "
           "choose which of the two you will fight. Attack your nominated "
           "target as in a normal battle. Against the other you will throw for "
           "your Attack Strength in the normal way, but you will not wound it "
           "if your Attack Strength is greater - you must just count this as "
           "though you have parried its blow. Of course if its Attack STrength "
           "is greater, it will have wounded you in the normal way.\n";

    std::cout << "\n\nHUNTING DOG A --- SKILL 6 --- STAMINA 6\n";
    std::cout << "\n\nHUNTING DOG B --- SKILL 5 --- STAMINA 6\n";
    Enemy hunting_dog1("HUNTING DOG A", 6, 6);
    Enemy hunting_dog2("HUNTING DOG B", 5, 6);
    battle(player, hunting_dog1, -1);
    battle(player, hunting_dog2, -1);

    std::cout << "\n\nHUNTING DOG C --- SKILL 6 --- STAMINA 5\n";
    std::cout << "\n\nMASKED MAN --- SKILL 8 --- STAMINA 7\n";
    Enemy hunting_dog3("HUNTING DOG C", 6, 5);
    Enemy masked_man("MASKED MAN", 8, 7);
    battle(player, hunting_dog3, -1);
    battle(player, masked_man, -1);
    page_62(player);
  } else {
    std::cout << "\n\nYou have perished against the Masked Man. Your adventure "
                 "ends here.\n";
    exit(0);
  }
...
void page_277(Player &player) {
  ...
  do {
    std::cout
        << "\n\nThe path continues east for some distance and then turns "
           "sharply north, narrowing slightly. The trees seem to crowd in on "
           "the path even more than usual and tingling down your spine tells "
           "you something is not right. Suddenly a branch from one of the "
           "trees to your right whips out and knocks you to the "
           "ground.\n\nLose 1 STAMINA point.\n\n You stagger to your feet to "
           "see that the tree has moved on the path, blocking your way. The "
           "tree is old, with thikc, cracked bark. You can just make out its "
           "hidden eyes and mouth in the middle of its trunk and realize you "
           "have been attacked by a TREEMAN. You may:\n1. Escape by running "
...
void page_278(Player &player) {
  std::cout
      << "\n\nThe path runs through a narrow gorge between two hills. You feel "
         "vulnerable and draw your sword, expecting to be ambushed at any "
         "moment. Unfortunately, because you are concentrating on watching the "
         "sides of the gorge, you do not see a small patch of leaves and "
         "branches on the path ahead. Your foot goes right through the thin "
         "covering of a bear trap, and you plunge four metres to the bottom of "
         "a rocky pit. To add to your misfortune, a wooden stake with a sharp "
         "tip point out of the centre of the pit.\n";
  ...
  if (playerRoll <= player.currLuck) {
    // LUCKY
    std::cout << "\n\nLucky! You manage to avoid landing on the wooden stake, "
                 "but fall heavily to the floor.\n\nLose 2 STAMINA points\n";
    player.removeLuck(1);
    player.removeStamina(2);
    next();
    page_319(player);
    ...
    // UNLUCKY
    std::cout << "\n\nUnlucky. The point of the stake pierces your leg as you "
                 "land.\n\nLose 2 STAMINA point for the fall, and a further 2 "
                 "STAMINA POINTS for injury to your leg.\n";
    player.removeStamina(4);
  }
  player.removeLuck(1);
  next();
...
void page_279(Player &player) {
  // TODO find the reference to this page and do the removals
  std::cout << "\n\nTreat all the items in your backpack as single objects, "
               "including each Gold Piece. Make the necessary deductions to "
               "your Equipment List.\n";
  next();
  page_246(player);
}
void page_280(Player &player) {
  std::cout
      << "\n\nYour walk west is arduous but uneventful. You pass two branches "
         "off the path leading south which you ignore. Eventually you arrive "
         "at a junction. Again ignoring the way south you head north.\n";
  next();
  page_306(player);
}
void page_281(Player &player) {
  ...
  do {
    std::cout
        << "\n\nYou arrive at a crossroads. South lead back up inot the hills "
           "so you decide against that opton.\n1. If you wish to head "
           "north\n2. If you wish to continue east.\n3. View Inventory\n";
...
void page_282(Player &player) {
  int pick = 0; // Declare pick outside the loop to persist between iterations
  do {
    std::cout << "\n\nThe red-topped fungi are being tended by three of the "
                 "humanoids.\n1. If you want to eat some of the fungus.\n2. If "
                 "you wish to leave the cavern by the steps on the far "
                 "wall.\n3. View Inventory\n";
...
void page_283(Player &player) {
  std::cout << "\n\nYour wounds heal before your eyes due to the magical "
               "properties of the mud.\n\nAdded 4 STMINA points to your score. "
               "Feeling much better you set off north again along the path.\n";
  next();
  page_303(player);
}
void page_284(Player &player) {
  int pick = 0; // Declare pick outside the loop to persist between iterations
  do {
    std::cout << "\n\nYou soon arrive at another junction in the tunnel.\n1. "
                 "If you wish to head north.\n2. If you wish to head "
                 "south.\n3. View Inventory\n";
...
void page_285(Player &player) {
  bool werewolf_wound = false;
  int starting_stamina = player.currStamina;
  std::cout
      << "\n\nYou have been asleep for about an hour when the noise of deep "
         "growling wakes you up. You stand up without making noise and grab "
         "your sword. You wait and listen. There is a full moon in the sky and "
         "the light casts eerie shadows followed by another low growl. Then a "
         "shape which looks like a man steps out of the shadows to your right; "
         "as he gets closer you see that his chest, arms and face are covered  "
         "with thick brown hair and long teeth protrude from his mouth. He is "
         "a WEREWOLF and you must fight him.\n";
  std::cout << "\n\nWEREWOLF --- SKILL 8 --- STAMINA 9\n";
  Enemy werewolf("WEREWOLF", 8, 9);
  battle(player, werewolf, -1);
  next();
  if (starting_stamina < player.currStamina) {
    werewolf_wound = true;
  }
  page_388(player, werewolf_wound);
}
void page_286(Player &player) {
  std::cout << "\n\nAs you raise your sword to make the first blow, the "
               "creature starts to metamorphose before your eyes. He becomes "
               "taller and turns green. A large spiny tail extends from his "
               "back, his arms thicken and his hands grow sharp claws. His "
               "face distorts and becomes reptilian with red eyes and a wide "
               "mouth housing dozens of razor-sharp teeth. He is not a Goblin "
               "but a SHAPE CHANGER, and you must fight him.\n";
  std::cout << "\n\nWEREWOLF --- SKILL 10 --- STAMINA 10\n";
  Enemy shape_changer("SHAPE CHANGER", 10, 10);
  battle(player, shape_changer, -1);
  next();
  page_373(player);
}
void page_287(Player &player) {

  std::cout << "\n\nOutside the cave you stop to take a quick look inside the "
               "leather bag."
               "You find:\n\n5 Gold Pieces\n\nBrass Bell\n";
  player.addGold(5);
  player.equipment.push_back({"Brass Bell", 2});
  next();
  page_25(player);
}
void page_288(Player &player) {
  ...
  do {
    std::cout
        << "\n\nWalking down the hill path you see the valley floor stretching "
           "out ahead and beyond that the sinister wall of trees - Darkwood! "
           "On the other side of the trees lies Stonebridge, you journry's "
           "end. As you reach the foot of the hills you see many large "
           "boulders lying around on either side of the path. You are amazed "
           "to see one of the larger ones gently rocking from side to side "
           "like a leaf in a breeze.\n1. If you wish to investigate the "
           "boulder\n2. If you would rather head north into the valley.\n3. "
           "View Inventory\n";
