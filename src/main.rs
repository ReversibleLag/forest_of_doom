                            ...
                            if let Ok(choice_index) = input.trim().parse::<usize>() {
                                if choice_index >= 1 && choice_index <= itemchoices.len() {
                                    ...
                                    if player.gold >= selected_item.price {
                                        ...
                                        if let Some(next) = &page.next {
                                            println!("\n\nHe tells you that all the instructions for use are written clearly on the labels attached to the items, together with their suggested use. He sighs and tell you that unfortunately the magic in the items only works once, but they are the best you can buy for the money.\n\nIf you decide to buy any of the items, pay for them by reducing the amount of Gold on your Inventory Sheet and add the items to the relevant sections on it. Yaztromo then asks you the reason for the purchase of the items, and you tell him your story and your decision to continue the quest of the luckless Bigleg. \"Ah yes\" Yaztromo say slowly, rubbing his chin, \"I heard that the good dwarfs of Stonebridge had lost their fabled war-hammer. Without it, their king is unable to arouse his people, despite the fact that the hill trolls threaten their village. Rumour has it that an envious king of another village of dwarfs sent and eagle to Stonebridge to steal the hammer, which it managed to do. But as it flew back over Darkwood, it was attacked by death hawks and the hammer dropped into the forest and was lost. Appearently, two forest goblins found the hammer but could not decide who was to keep it. They wrestled fo hours but gave up. Then they discovered that the handle unscrewed from the head, and the arguement was settled. One kept the head, the other kept the handle. Then they parted, each happy with his new treasure. Who knows if they still have them. So I'm afraid your problems are doubled. I can tell you that the head is made of bronze and the handle is made of polished ebony. Both head and handle have the letter G inscribed on them. Your task is not easy. Good luck.\"\n\nYou thank Yaztromo and leave the room by the spiral staircase.");
                                            current_page = *next;
                                            break;
                                        }
                                    }
